...
class VirtualPortfolioManager:
    ...
    def buy_stock(self, code, name, price, quantity):
        ...
        if balance['cash'] < actual_cost:
            print(f"[VPM] 🚫 {name} 매수 거부: 예수금 부족 (필요: {actual_cost:,.0f}원)")
            return None
        ...
        print(f"[VPM] ✅ {name} {quantity}주 매수 완료 (비용: {actual_cost:,.0f}원)")
        return port[code]
    ...
    def sell_stock(self, code, current_price, sell_qty=None):
        ...
        # 가상 잔고에 매도 대금 입금 (현재가 기준 정산)
        sell_amount = qty_to_sell * current_price
        balance['cash'] += sell_amount
        ...
        self.save_all(port, balance)
        print(f"[VPM] 🚨 {code} {qty_to_sell}주 매도 완료 (정산 대금: {sell_amount:,.0f}원)")
        return {"sold_qty": qty_to_sell, "sell_amount": sell_amount}
