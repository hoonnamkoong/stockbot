...
class GeminiAgent:
    ...
    def analyze_batch_discovery(self, batch_data):
        ...
        for i in range(0, len(pending), GROUP_SIZE):
            ...
            1. 감정적 단어(가즈아, 존버, 떡상 등) 및 선동 문구는 **fact_score에서만**
               무조건 배제하고 0점 처리한다. sentiment 판단에는 그대로 쓴다(7번 참고) —
               감정적 표현이야말로 논조를 보여주는 재료다.
            2. 다음 핵심 키워드를 반드시 우대하여 fact_score 분석에 포함: '분석', '추세', '전망', '공시', '뉴스', '따르면', '의하면'.
            ...
            7. sentiment(-10~10)는 '게시글들이 이 종목에 대해 얼마나 방향성 있게
               긍정적/부정적인가'다 — 근거(공시·실적 등) 유무와 무관하다.
               근거 없는 순수 기대·소문이라도 논조가 뚜렷하게 긍정적이면 sentiment는
               높게(양수), 근거가 확실해도 논조가 부정적(실적 악화 확정 등)이면
               낮게(음수) 매긴다. fact_score와 반드시 같은 방향일 필요가 없다 —
               "근거는 있는데 나쁜 소식"과 "근거는 없는데 좋은 소식"을 구분하는 게
               이 점수의 존재 이유다.
