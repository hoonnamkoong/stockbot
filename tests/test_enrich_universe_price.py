...
라이브 price를 네이버 수급 표(일봉) 값으로 조건 없이 덮어쓴 것.
그 표는 장중에 갱신이 멈출 수 있어(Sim6가 6주간 거래 0건이던 것과 동일 함정),
...
def _trend(price=6710):
    """네이버 일별 수급 행(naver_api.investor_trend) — 최신이 앞, 첫 행이 '오늘' 종가."""
    def row(date, close):
        return {'date': date, 'close': close, 'volume': 1, 'organ_net': 10,
                'foreign_net': 20, 'foreign_hold_ratio': 3.4}
    return [row('20260804', price), row('20260803', 6500)]


def _enrich_with_naver(stocks, naver_price=6710):
    kis = mock.MagicMock()
    ...
    with mock.patch('src.data.naver_api.investor_trend', return_value=_trend(naver_price)), \
...
def test_enrich_does_not_overwrite_live_price_from_get_universe():
    ...
    네이버 수급 표의 값으로 덮어쓰면 안 된다 — 그게 오늘의 버그였다."""
    stocks = [{'code': '317400', 'name': '자이에스앤디', 'price': 7680,
               'current_price': 7680}]
    out = _enrich_with_naver(stocks, naver_price=6710)
...
def test_enrich_fills_price_when_universe_has_none():
    ...
    out = _enrich_with_naver(stocks, naver_price=1314)
