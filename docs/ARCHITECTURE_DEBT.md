...
| `8cbc251d` `e349c171` | 6순위 셋째 조각 — 실전 계좌 요약 숫자 + 실버그 하나(1,040 → 811줄) |
...
### 6순위 — `TradeClient.tsx` 분해 (1,040 → 811줄) ← **세 조각 완료, 여기서 이어간다**
...
| `renderRealPortfolioSection` | 410–619 | JSX만 남았다(요약 계산은 lib으로 내려갔다). 프로그램 ON/OFF·PIN·일괄매도 |
| `renderTrading` | 672–804 | 주문 폼 |

다음 조각으로 뭘 잡든 순서는 같다: **순수 계산을 `src/lib`로 내려 테스트 → 컴포넌트
파일을 먼저 커밋 → 그다음 배선.** 실거래 문 구간은 [[program-trading-parity-mandate]]
때문에 눈으로 한 번 확인하고 넘어가는 것이 낫다.
