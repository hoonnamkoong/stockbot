...
두 트랙의 합계는 자연히 일치한다. 3,000원에 사서 턴1이 3,500원에 끝나고 턴2가 3,500원에 시작해 3,700원에 팔면 — 턴1 +500, 턴2 +200, 합 +700 = 누적 실현손익 +700.

단 이 일치는 **OFF 시점 시세와 다음 ON 시점 시세가 같을 때만** 성립한다. OFF 동안 시세가 움직이면 그 변동분은 어느 턴에도 귀속되지 않아 턴 손익의 합이 누적 실현손익보다 작거나 크다. 이는 버그가 아니라 아래 규칙 2의 의도다 — 프로그램이 돌지 않은 구간은 어느 턴의 성과도 아니다. (턴 히스토리를 보관하지 않으므로 이 합을 화면에 표시하지도 않는다.)
