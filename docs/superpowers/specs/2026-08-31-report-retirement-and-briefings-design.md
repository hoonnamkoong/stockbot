...
대상이다. 이 블록이 읽는 `sync_state.morning_reported_info` 등은 다른 소비자가
있으므로 필드 자체는 남긴다.
...
2. **당일 15:00 KST** — 마감 브리핑이 현행 문구·수치 그대로인지(회귀 없음)
3. **당일 11:00·14:00 KST** — 리포트가 **오지 않는지**
4. **익일 09:00 KST** — 미국 마감 브리핑 도착. `us_brief_gate_state.json`이
   db-data에 커밋돼 09:00~09:40 사이 중복 발송이 없는지 확인
5. **익일 07:20 KST** — premarket intraday 잡이 gzip으로 push 성공하는지
   (`data/rt_intraday_*.csv.gz`가 db-data에 처음 올라오는지)
