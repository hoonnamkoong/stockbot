...
**발화 경로가 둘이다(2026-09-16).** 예전엔 네이티브 cron 하나였는데 하루 7회
기대 대비 **장중 유효 1회(6/42 = 14.3%)**로 드롭돼 09:00~11:30과 12:00~15:30이
무감시였다. 그래서 태스커→Vercel `/api/cron` 경로를 **주 발화**로 올려 09:15~15:50
창을 30분 격자로 덮고(세션당 13회), cron은 백업으로 남겼다.

cron을 지우지 않은 이유가 중요하다 — **주 경로는 감시 대상(trading.yml)과 같은
폰을 공유한다.** 폰이 멈추면 감시자와 감시 대상이 같이 조용해진다. cron은 그와
무관한 유일한 발화라 장중 1회뿐이어도 0과 1은 다르다. 폰 사망 자체는
data_audit_backup.yml(13:00 KST 네이티브 cron)이 더 느리게 잡는다.

⚠ **격자를 더 좁히려면 쿨다운이 먼저다.** 이 스크립트는 `send_alert`를 쓰고
`send_alert_once`의 쿨다운을 타지 않아 **격자가 곧 고장 1건당 텔레그램 통수**다
(30분 = 13통, 15분 = 27통).
"""
