...
class DonchianBreakoutSimulator(BaseSimulator):
    ...
    - ⚠ 과거 검증 불가(tradeable: false). 월별 엑셀은 유니버스가 동적이라 종목당 평균
      4.9일만 관측돼 20일 채널 신호가 41거래일 전체에서 4건뿐이다. 페이퍼 관찰 단계.
    - ⚠ 원 터틀은 고가/저가 기준인데 여기서는 종가 기준이다(보유 데이터의 한계).
      종가 돌파가 고가 돌파보다 엄격하므로 신호가 덜 나는 쪽으로 보수적이다.
      장 마감 후 런에서는 range_history에 당일 종가가 포함돼 돌파가 성립하지 않을 수
      있다 — 신호를 놓치는 방향이라 그대로 둔다. KIS 일봉 백필 시 함께 해소된다.
    """
