...
| `0f11ee18` `c13cc51d` | 6순위 첫 조각 — TradeClient에서 표 두 개 분리(1,040 → 889줄) |

지금 상태: pytest 463 · node 80 · push마다 CI 자동 실행.
**남은 것은 6순위(TradeClient 분해)의 나머지다.** 아래 6순위 절에 남은 구간 표가 있다.
...
### 6순위 — `TradeClient.tsx` 분해 (1,040 → 889줄) ← **첫 조각 완료, 여기서 이어간다**
...
**남은 것** (`useState` 41개는 그대로 — 전부 최상단 `TradeContent`에 있다):

| 구간 | 줄 | 성격 |
|---|---|---|
| 상태·페처·핸들러 | 54–409 | `useState` 41개. 쪼개려면 훅으로 묶어야 한다 |
| `renderRealPortfolioSection` | 410–639 | **실거래 문**(프로그램 ON/OFF·PIN·일괄매도). 판정은 이미 `trade-auth.ts`에 테스트가 있다 |
| `renderSimulationTripod` | 640–749 | 심 카드. 순수 표시라 다음으로 안전한 조각 |
| `renderTrading` | 750–882 | 주문 폼 |
