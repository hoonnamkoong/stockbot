...
# 매입가 복귀 손절. +5% 분할익절을 이미 찍은(모멘텀이 한 번 검증된) 종목이 본전으로
# 되돌아오면 남은 수량을 정리한다.
#
# 2026-08-20에 버퍼 -1.5%를 뒀다가 **2026-09-16에 0으로 원복했다.** 사후 측정 결과:
#
#   비용은 기계적이다 — BE손절이 걸리려면 가격이 0%를 반드시 먼저 통과하므로
#   버퍼 폭만큼 매번 확정적으로 나쁜 가격에 나간다. 실측 8건 평균 -1.60pp.
#   편익은 확률적이다 — 밴드에서 되돌아 나와야 한다. 밴드 진입 약 9건 중
#   +10% 2차익절까지 회복 0건, 어떤 형태로든 플러스 청산 1건(11%)으로
#   손익분기 필요확률 14.2%에 미달. 순 -7.9pp / 19포지션(실계좌 9거래일 약 -13,040원).
#   표본이 작을 때 확정 비용과 저확률 편익을 맞바꾼 구조라 기댓값이 음(-)이었다.
#
# 완화의 원래 관찰("며칠 더 버텼으면 더 나았다")은 완화 후에도 재현된다
# (페이퍼 T+1 71%, 평균 +6.8pp). 틀린 건 관찰이 아니라 **폭**이었다 — 이 포지션들을
# 실제로 죽이는 되돌림 깊이는 -2~-4%대라 -1.5%는 "넓은 손절의 비용만 내고 편익은
# 못 받는" 자리였다. 그 패턴을 잡으려면 가격 완충이 아니라 **시간 완충**(연속 N사이클
# 0% 하회 요구)이어야 하고, 그건 별도 설계다.
#
# ⚠ 08-20 완화의 근거였던 "9건 중 67~88%, 평균 +2.7~+14.2pp"는 **재현 불가**다 —
# db-data에 2026-07 가격 데이터가 없어 9건 중 6건을 측정할 수 없고, 재현 가능한
# 3건 중 1건은 T+1 -11pp/T+5 -16pp다. 산출 스크립트도 안 남아 있다.
#
# 순수 하드손절(-3%)은 그대로 둔다 — 그쪽 30건 중 21건은 즉시 반등 근거가 약했다.
