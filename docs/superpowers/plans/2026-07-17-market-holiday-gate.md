...
**2026-07-17 20:46 KST, `token_refresh.yml` 수동 실행으로 검증 완료.**

- [x] `token_refresh.yml` 런 로그: `[MarketCalendar] 24일치 저장 완료. 오늘(20260717) 개장여부=N` ✅
- [x] db-data에 `data/market_calendar.json` 생성 (커밋 `07f8142`) ✅
- [x] **`opnd_yn` 필드명이 실제 응답에 존재** ✅ — 라이브 확인됨. 문서 스펙이 맞았다.
- [x] **오늘(20260717 금)이 `N`(휴장)으로 판정** ✅ — 이번 사고의 그 날짜. KIS는 제헌절을
      정확히 알고 있다(`holidays` 0.86이 몰랐던 것). 주말 9일 전부 `N`, 나머지 평일 15일 전부 `Y`.
- [ ] 거래일 스크래퍼 런에서 `[휴장 판정] ... 개장=True` 로그가 찍히고 파이프라인이 정상 진행되는지
      → **다음 거래일(2026-07-20 월) 확인 예정**
- [ ] 다음 휴장일에 스크래퍼가 즉시 종료되고 텔레그램이 오지 않는지

### 실측으로 드러난 차이: "3개월치"가 아니라 24일치

`BASS_DT=20260717` 요청에 **24일치**(`20260717`~`20260809`, 23일 스팬)가 왔다. 계획·스펙의
"약 3개월치"는 문서 스펙 추정이었고 실제와 다르다. 최종 리뷰가 이를 예견했다
(`parse_calendar`가 `tr_cont` 페이징을 무시 → 첫 페이지가 전부).

**판정에는 무관하다** — `BASS_DT`를 오늘로 요청하므로 오늘은 언제나 첫 페이지에 있다.
07시 런이 매일 갱신하니 3주 앞이 늘 채워져 있고, 연속 실패가 그 범위를 넘겨야 달력이
만료되는데 그때는 스크래퍼의 직접 재조회 계층이 받는다. 주석·문서를 실측에 맞춰 정정했다.
