...
class GeminiAgent:
    ...
    def generate_trading_guide(self, market_context, sentinel_signals):
        ...
        # [V8.4.1] 상위 5개 데이터 다이어트 (본문 200자 정밀 슬라이싱)
        lightweight_top_5 = []
        for s in top_5:
            mini_s = s.copy()
            if 'latest_posts' in mini_s:
                # [지시사항] 반드시 게시글 본문(body)을 200자로 자르는 리스트 컴프리헨션 적용
                mini_s['latest_posts'] = [
                    {**p, 'body': str(p.get('body', ''))[:200]} 
                    for p in mini_s.get('latest_posts', [])
                ]
            lightweight_top_5.append(mini_s)

        # [V8.4.1] 마크다운 강제 및 HTML 금지 프롬프트 고도화
        prompt = f"""
        당신은 공격적인 퀀트 분석가이며 '인사이트 중심 딥다이브' 리포트의 수석 에디터입니다. 
        아래 제공된 [실제 데이터]만을 사용하여 리포트의 본문을 작성하세요. 
        ...
        2. Top 5 Deep-Dive: 상위 5개 종목 각각에 대해 아래 4개 요소를 구체적 데이터 기반으로 서술.
           ...
           - 커뮤니티 심층 분석 (Community Insight): 본문의 핵심 논점 요약.
           - 공시/뉴스 팩트 체크: 버즈와 일치하는 실재하는 텍스트 인용.
           - AI 최종 판단: [ML 확률 + 감성 점수]를 수치로 표기하고 '진짜 호재' 여부 판정.
        3. 나머지 10개 종목: [종목명 | AI 점수 | 핵심 키워드] 위주로 리스트화.
        
        🚨 중요 지시사항 (반드시 준수):
        - 텔레그램 파싱 에러를 막기 위해 HTML 태그(<b>, <i> 등)를 절대 사용하지 마라.
        - 강조가 필요할 경우 오직 마크다운(**텍스트**)만 사용하여 작성하라.
        - 템플릿 대괄호([]) 형식을 그대로 출력하지 마라.
        - 오직 전문가용 불렛포인트(*)와 핵심 키워드 중심의 실전 리포트만 출력하라.
        """
        try:
            response = self.model.generate_content(prompt)
            if response and response.text:
                res_text = response.text
                # [V8.4.1] 반환 전 정규식으로 모든 HTML 태그를 한 번 더 제거 (강제 정화)
                res_text = re.sub(r'<[^>]*>', '', res_text)
                # HTML 엔티티 치환 (가독성 확보)
                res_text = res_text.replace('&lt;', '<').replace('&gt;', '>').replace('&amp;', '&').replace('&quot;', '"').replace('&nbsp;', ' ')
