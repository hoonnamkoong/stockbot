"""브리핑 발송 슬롯 게이트 — 하루 2회(12:00·15:00)를 놓치지 않는다.

왜 슬롯 게이트가 필요한가: 예전 판정은 `PipelineContext.should_notify()`의
"파이썬 시작 분이 0~2인가" 하나였다. 그 값은 디스패치 + 큐 대기 + 셋업(약 50초)
뒤의 시각이라 런마다 몇 초씩 흔들린다. 2026-08-07에 격자가 밀리면서 정각 발송이
...
2026-08-31에 11:00·14:00 리포트 슬롯을 폐기했다. 위 판정 방식은 남은 브리핑
슬롯이 그대로 물려받는다 — 놓치면 안 되는 이유가 똑같기 때문이다.
...
def test_closed_before_the_first_slot(tmp_path):
    assert gate.brief_due(_at(11, 59), str(tmp_path)) is None


def test_opens_at_the_slot_time(tmp_path):
    assert gate.brief_due(_at(12, 0), str(tmp_path)) == '12:00'


def test_stays_open_through_the_retry_window(tmp_path):
    """스크래핑은 10분 격자다. 12:00 트리거를 놓쳐도 :10 :20 :30에 다시 온다 —
    한 번의 셋업 지터가 그날 회차를 통째로 없애면 안 된다."""
    for m in (0, 9, 17, 33, 39):
        assert gate.brief_due(_at(12, m), str(tmp_path)) == '12:00', f'12:{m:02d}'


def test_expires_after_the_window(tmp_path):
    """12:40을 넘겨 보내면 '12시 브리핑'이 아니다. 늦은 브리핑보다 없는 편이 낫다."""
    assert gate.brief_due(_at(12, 41), str(tmp_path)) is None


def test_second_slot_is_independent(tmp_path):
    d = str(tmp_path)
    gate.mark_sent('12:00', _at(12, 2), d)

    assert gate.brief_due(_at(12, 20), d) is None, '이미 보낸 슬롯이 다시 열렸다'
    assert gate.brief_due(_at(15, 3), d) == '15:00'


def test_a_new_day_reopens_the_slots(tmp_path):
    """상태가 db-data를 왕복하므로 어제 기록이 남아 있다. 날짜로 갈라야
    다음 날 브리핑이 통째로 사라지지 않는다."""
    d = str(tmp_path)
    gate.mark_sent('12:00', _at(12, 2, day=10), d)

    assert gate.brief_due(_at(12, 2, day=11), d) == '12:00'
...
def test_unreadable_state_opens_the_gate(tmp_path):
    ...
    assert gate.brief_due(_at(12, 5), str(tmp_path)) == '12:00'
...
def test_marking_both_slots_accumulates(tmp_path):
    ...
    assert raw['sent'] == ['12:00', '15:00']


def test_naive_datetime_works_too(tmp_path):
    """PipelineContext.now_kst는 tz 없는 KST다. 게이트가 그걸 그대로 받아야 한다."""
    assert gate.brief_due(datetime(2026, 8, 10, 12, 5), str(tmp_path)) == '12:00'


# ── 15시 마감 브리핑도 자기 슬롯을 갖는다 ──────────────────────────
# 12시는 09:00~12:00 구간, 15시는 하루 전체 마감이다. 두 슬롯은 서로 독립이라
# 12시를 보냈다는 사실이 15시를 막지 않는다 — 상수 하나로 닫으면 15시가 사라진다.
...
def test_slots_are_the_agreed_times():
    """12시·15시 합의(2026-08-31). 바꾸면 브리핑 회차가 조용히 달라진다."""
    assert gate.BRIEF_SLOTS == ('12:00', '15:00')
