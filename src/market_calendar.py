...
chk-holiday(TR CTCA0903R)는 BASS_DT 하루가 아니라 여러 날치 달력을 한 번에
반환한다. 응답을 통째로 저장해두면 재조회가 거의 필요 없고, 07시 갱신 런이
하루 실패해도 이전 저장분에 오늘이 들어있다.

[실측 2026-07-17] BASS_DT=20260717 요청에 24일치(20260717~20260809)가 왔다.
tr_cont 페이징을 하지 않으므로 첫 페이지가 전부다. 판정에는 무관하다 —
BASS_DT를 오늘로 요청하므로 오늘은 언제나 첫 페이지에 있다. 07시 런이
매일 갱신하니 실무상 3주 앞이 늘 채워져 있고, 연속 실패가 그 범위를 넘겨야
달력이 만료되는데 그때는 스크래퍼의 직접 재조회 계층이 받는다.
